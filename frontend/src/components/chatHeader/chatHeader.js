...
const ChatHeader = ({ handleUserModal, setHidden, setShowUser }) => {
  ...
  const search_channels = channels?.filter((channel) =>
    channel?.name.toLowerCase().includes(search?.toLowerCase())
  );

  const search_users = users?.filter((user) =>
      ...
      <NavLink to={`/channels/${channel.id}`}>
        <div
          className={`search-result-div ${
            sessionUser.darkMode ? "search-result-div-dark" : ""
          }`}
        >
          <span id="hash">#</span>
          {channel.name}
  ...
  const display =
    channel_list.length === 0 && user_list.length === 0 ? (
      <ul
        className={`search-results ${
          sessionUser.darkMode ? "search-results-dark" : ""
        }`}
      >
    ...
    <div className={`header-div ${sessionUser.darkMode ? "header-dark" : ""}`}>
      <div className="links">
        ...
        <a
          ...
          rel="noreferrer"
        >
          <img id="logo" alt="linkedin" src={li} />
        </a>
        <a
          ...
          rel="noreferrer"
        >
          <img id="logo" alt="angellist" src={al} />
        </a>
      </div>
      <div className={`search-div`}>
        <input
          placeholder={`🔍︎ Search ${search === "" ? "server" : search}`}
          className={`searchBar ${sessionUser.darkMode ? "search-dark" : ""}`}
          value={search}
